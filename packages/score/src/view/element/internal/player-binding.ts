...
export interface ViewPlayerHandlers {
  ...
  /** Observe source readiness independently of its projected cursor. */
  sourceSnapshot?(snapshot: ScorePlaybackSnapshot): void;
  sourceError?(cause: unknown): void;
...
export function readViewPlayerScore(target: Element | undefined, source?: ScorePlaybackSource): Score | undefined {
  const player = target as ViewPlayer | undefined;
  const native = (source ?? player?.playback)?.snapshot();
...
export function bindViewPlayer(
  host: Element, handlers: ViewPlayerHandlers, selectorAttribute = 'player', source?: ScorePlaybackSource,
): (() => void) | undefined {
  const selector = host.getAttribute(selectorAttribute);
  if (!selector && !source) return undefined;
  ...
  const attach = (next: Element | undefined, state?: ElementTargetState): void => {
    if (disposed || (!source && host.getAttribute(selectorAttribute) !== selector)) return;
    ...
    const current = () => !disposed && ownGeneration === generation && (!wasConnected || host.isConnected)
      && (Boolean(source) || host.getAttribute(selectorAttribute) === selected) && handlers.isCurrent?.() !== false;
    ...
    const native = source ?? (next as ViewPlayer | undefined)?.playback;
    ...
    const acceptsLegacy = () => !source && (!native || native.snapshot().readiness === 'unavailable');
    const snapshot = (): ViewPlayerSnapshot | undefined => readViewPlayerSnapshot(next, source);
    ...
    const publishScore = (force = false): void => {
      if (!current()) return;
      const score = readViewPlayerScore(next, source);
    ...
    if (native && typeof native.subscribe === 'function') {
      const candidate = observeScorePlayback(native, (state) => {
        ...
        try {
          ...
          if (!current()) return;
          if (handlers.sourceError) handlers.sourceError(cause);
          else throw cause;
        }
      });
  ...
  try {
    ...
    if (source) {
      attach(undefined, 'ready');
    } else if (typeof root.nodeType === 'number') {
...
export function readViewPlayerSnapshot(target: Element | undefined, source?: ScorePlaybackSource): ViewPlayerSnapshot | undefined {
  const native = (source ?? (target as ViewPlayer | undefined)?.playback)?.snapshot();
...
export function seekViewPlayer(target: Element | undefined, seconds: number, score?: Score, rate = 1, playback?: ScorePlaybackSource): void | Promise<void> {
  ...
  const source = playback ?? player?.playback;
  if (source && (playback || source.snapshot().readiness !== 'unavailable')) {
