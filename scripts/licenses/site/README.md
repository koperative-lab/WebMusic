...
| [OpenSheetMusicDisplay 2.1.3](opensheetmusicdisplay-2.1.3.json) | The official release JavaScript again matches the npm artifact after removing its final sourceMappingURL comment. Comparing its source map with the retained 1.9.9 map finds the same 100 npm-source modules: 93 are byte-identical, including JSZip and its nested dependencies, loglevel, TypeScript Collections and VexFlow's font data. Seven VexFlow modules contain OSMD patches with unchanged copyright headers; the 14 added modules are OSMD's own geometric context and Braille plugin. The JSON records both map hashes, changed-source hashes and font evidence. The [bundled notices](opensheetmusicdisplay-2.1.3-bundled.txt) preserve the earlier license texts and Gonville provenance; the package's BSD-3-Clause license covers its own additions. |
