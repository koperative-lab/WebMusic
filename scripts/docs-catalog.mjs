...
const OUTPUTS = ['dev/COMPONENTS.md', 'dev/DOCUMENTATION-MAP.md'];
...
function documentationFiles() {
  // Include tracked and non-ignored files, including AGENTS.md, dev/ and
  // .agent/. Ignored personal instructions and build output stay out.
  ...
  return [...new Set(files)].filter((file) => {
    ...
    return lstatSync(path.join(ROOT, file), {throwIfNoEntry: false})?.isFile();
  }).sort();
}
...
const isCurrentDevGuide = (file) => file.startsWith('dev/') &&
  (!DEV_HISTORY.test(file) || file.endsWith('/README.md'));
const isMaintainedGuide = (file) => isCurrentDevGuide(file) || file.startsWith('.agent/');
...
function role(file) {
  ...
  if (file.startsWith('.agent/')) return 'Project agent toolkit / workflow / rule-owner map; accepted contracts remain in dev';
  ...
  if (file.startsWith(`${DOCS}/`)) return 'Public reference / orientation; page type governed by dev templates';
  if (file.startsWith('dev/') && file.endsWith('/README.md')) return DEV_HISTORY.test(file)
...
function documentationMap(files) {
  ...
  for (const file of files) {
    const group = !file.includes('/') ? 'Project and agent entry points'
...
function currentLinkSource(file, source) {
  ...
  const archive = /^(?:dev\/plans\/|packages\/[^/]+\/docs\/|scripts\/release-pipeline\/)/.test(file);
  ...
  if (!archive || path.posix.basename(file) === 'README.md') return prose;
...
function fileLinkProblems(files, {readFile = read, targetExists = exists} = {}) {
  ...
  for (const file of files) {
    ...
    for (const target of markdownFileTargets(source)) {
      ...
      if (!targetExists(posix(path.normalize(path.join(path.dirname(file), pathname))))) failures.push(`${file}: missing local target ${target}`);
    }
  }
  return failures;
}
...
function devNavigationProblems(files, {readFile = read} = {}) {
  const guides = new Set(files.filter(isMaintainedGuide));
  const reachable = new Set();
  const pending = ['AGENTS.md'];
...
function runDevNavigationProbes() {
  ...
  const probes = [
    ...
    {name: 'unused reference definition is not navigation', docs: {
      'AGENTS.md': '[unused]: dev/design/component.md', [guide]: '',
    }, expected: orphan},
    {name: 'inline code is not navigation', docs: {
      'AGENTS.md': '`[Component](dev/design/component.md)`', [guide]: '',
    }, expected: orphan},
    ...
    }, expected: ['.agent/skills/review/SKILL.md: no maintained reading route from AGENTS.md; link it from the appropriate dev or .agent index']},
  ];
...
function runFileLinkProbes() {
  ...
  for (const probe of probes) {
    const result = fileLinkProblems([probe.file ?? ordinary], {
      ...
      targetExists: (file) => file.endsWith('/exists.md'),
    });
...
if (process.argv.includes('--self-test')) {
  console.log(`Documentation probes passed: ${probeCount} file-link cases, ${navigationProbeCount} navigation cases.`);
} else {
  assert(lstatSync(path.join(ROOT, 'AGENTS.md'), {throwIfNoEntry: false})?.isFile(),
    'AGENTS.md must exist as the canonical instruction file.');
  const files = [...new Set([...documentationFiles(), ...OUTPUTS])].sort();
  assert(files.includes('AGENTS.md'), 'AGENTS.md must be included in the repository documentation inventory.');
  const outputs = new Map([[OUTPUTS[0], await componentIndex(files)], [OUTPUTS[1], documentationMap(files)]]);
  if (process.argv.includes('--write')) {
    for (const [file, body] of outputs) writeFileSync(path.join(ROOT, file), body);
    console.log(`Documentation inventories generated: ${files.length} documents, ${UI_COMPOSITION_CATALOG.length} elements, ${UI_PRESENTER_CATALOG.length} presenters.`);
  } else {
    const failures = [];
    for (const [file, body] of outputs) if (!exists(file) || read(file) !== body) failures.push(`${file}: stale or missing; run npm run docs:sync`);
    failures.push(...fileLinkProblems(files.filter(exists)));
    failures.push(...devNavigationProblems(files.filter(exists)));
    if (failures.length) {
      console.error(`Development documentation check failed:\n${failures.map((f) => `- ${f}`).join('\n')}`);
      process.exitCode = 1;
    } else console.log(`Development documentation check passed: ${files.length} documents; generated inventories, local file links and dev/.agent reading routes are consistent.`);
  }
}
